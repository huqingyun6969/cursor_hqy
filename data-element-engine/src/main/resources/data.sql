...
('ENCODING_RULE', '编码规则校验', 'FIELD',
 ...
 1, '{"pattern":"^\\\\d{6}$"}',
 '{"pattern":"编码正则表达式，如 ^\\\\d{6}$","prefix":"编码前缀(可选)","length":"编码长度(可选)"}', 1, 14);
...
('CUSTOM_SQL', '自定义SQL校验', 'FIELD',
 ...
 1, '{"condition":"IS NOT NULL"}',
 '{"condition":"条件表达式，如: IS NOT NULL / LENGTH(field) > 5 / IN (''男'',''女'') / LIKE ''%@%''"}', 1, 18);
...
('SCRIPT', '动态脚本校验', 'FIELD',
 ...
 1, '{"scriptLanguage":"java","scriptBody":"见示例代码"}',
 '{"scriptLanguage":"脚本语言: java 或 js","scriptBody":"在规则定义的代码编辑器中编写"}', 1, 19);
