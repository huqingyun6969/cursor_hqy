...
('CUSTOM_SQL', '自定义SQL校验', 'FIELD',
 '【用途】通过自定义条件表达式对每行数据进行校验\n【适用场景】内置规则无法满足的复杂校验逻辑\n【使用方式】在规则的「自定义SQL」字段中编写条件表达式\n【条件表达式说明】(作用于每行数据，返回TRUE表示通过)：\n  - IS NOT NULL: 字段不为空则通过\n  - IS NULL: 字段为空则通过\n  - LENGTH(field) > N: 字段长度大于N则通过\n  - LIKE ''pattern'': 字段值匹配模式则通过\n  - IN (''v1'',''v2''): 字段值在列表中则通过\n  - COUNT: condition: 计数模式，满足条件的行算作违规\n【示例1】字段长度大于5: LENGTH(field) > 5\n【示例2】值在指定范围: IN (''A'',''B'',''C'')\n【示例3】计数违规(空值): COUNT: IS NULL',
 1, NULL, NULL, 1, 18);

INSERT IGNORE INTO dep_rule_type_config (type_code, type_name, rule_level, description, needs_params, default_params, param_template, status, sort_order) VALUES
('SCRIPT', '动态脚本校验', 'FIELD',
 '【用途】通过JavaScript或Java动态脚本实现完全自定义的校验逻辑\n【适用场景】复杂业务规则、跨字段校验、外部接口调用等内置规则无法满足的场景\n【使用方式】\n  1. 新建规则时选择「动态脚本校验」类型\n  2. 选择脚本语言(Java推荐 / JavaScript)\n  3. 在代码编辑器中编写校验逻辑\n  4. 可点击「插入示例代码」查看模板\n【脚本引擎】\n  - Java: javax-pro引擎，完整Java语法，可import任意类\n  - JavaScript: GraalJS引擎，支持ES6语法\n【关键API】\n  RuleContext ctx = this.getContextBean(RuleContext.class);\n  ctx.getCurrentRule() — 获取当前规则定义\n  ctx.getDataRows() — 获取数据行(List<Map>)\n  ctx.setViolatedRows(count) — 设置违规行数\n  ctx.addViolation(msg) — 添加违规样本(最多10条)',
 1, NULL, NULL, 1, 19);
